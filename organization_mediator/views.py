...
"""
Settings for openhim Organization mediator developed in Django.

The python-based Organization mediator implements python-utils 
...
from .models import Organization # import org model class
...
def getOrganization(request):
	...
	if request.method == 'GET':
		openimisurl = configurations["data"]["openimis_url"]+str(os.environ['ORGANIZATION'])
		querystring = {"":""}
		...
		try: # attempt opening the default config json file from hidden conf directory
			...
			response = requests.get(baseurl+'/{orgurl}'.format_map(
				params[2]),verify=False) # get resource uri from 2nd array in .conf/PathParaheters.json
		except (FileNotFoundError,IOError) as e: # else bypass import and use django admin
			pass

		mifosurl = response.url #mifos endpoint
		organizationurl = response.url+f"?fields={os.environ['CLIENTQUERYLIST']}" # use f to allow comma seperate list on URL
		...
		try: # this captures JSON decode exception caused by invalid API endpoint
			...
			keymap = {  # openIMIS Group to MIFOS Organization mapping dictionary
						'id':'externalId',
						'name':'fullname',
			...
			Organization attributes to MIFOS X attributes. The first task is to call transform_merged_subdictionaries that slices the nested 
			...
			payload_org = json.loads(payload)
			...
			org_data = requests.request("GET", organizationurl, data=request.data, headers=headers, params=querystring,verify=False)
			payload_org = json.loads(org_data.text) # extract the payload part of the response
			save_orgdata = mediators_save_organization(payload_org)	# call this method that save the json payload into the database
...
def transform_merged_subdictionaries(data_dict):
    ...
    if key_to_lookup in data_dict:				
        ...
        data_dict[key_to_lookup][0]['street']=data_dict[key_to_lookup][0]['line'][0]	
        data_dict[key_to_lookup][0].pop('line')  
...
openIMIS Organization keys with those defined in the keymap. The keymap is a dictionary that maps Organization keys to MIFOS 
...
def mediators_save_organization(orgdata): 
	for child in orgdata['pageItems']: #iterate to display all objects in the json array
...
def registerOrganizationMediator():
	result = configview()
	configurations = result.__dict__
	try: 
		...
		conf = {
		"urn": "urn:mediator:openhim-mediator-python-mifos-organizations",
		"version": "1.0.1",
		"name": "FHIR R4 Organization Mediator",
		"description": "Python FHIR R4 Organization Mediator",

		"defaultChannelConfig": [
			{
				"name": "FHIR R4 Organization Mediator",
				"urlPattern": "^/fineract-provider/api/v1/organizations$",
				"routes": [
					{
						"name": "FHIR R4 Client Mediator Route",
						"host": host,
						"path": "/fineract-provider/api/v1/organizations",
		...
		"endpoints": [
			{
				"name": "Organization FHIR R4 Mediator",
				"host": host,
				"path": "/fineract-provider/api/v1/organizations",
				"port": configurations["data"]["mediator_port"],
				"primary": True,
